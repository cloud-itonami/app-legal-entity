(ns legal-entity.worker
  "Cloudflare Worker の入口。**この appview で唯一 Request/Response に触る層。**

  ここには判断を置かない —— どのハンドラが答えるかは
  `legal-entity.route/dispatch` が決め、ページの中身は `legal-entity.view` が
  組む。どちらも `.cljc` なので、ブラウザもビルドも無しにテストできる。

  `wasm/etzhayyim-wasm-legal-entity-le9k4x2m/wrangler.jsonc` の `main` は
  `../../dist/worker.js` を指し、それはこの名前空間をコンパイルしたものである。
  移行前は `svelte/.svelte-kit/cloudflare/_worker.js` —— tree に存在しない
  ビルド出力 —— を指していて、読み手が開く `src/app.ts` はどの config からも
  参照されていなかった（docs/adr/0001）。

  `aget` を使うのは `:advanced-optimization` 下で env のキーが潰れないため
  （先例 `listingops.edge.worker` と同じ約束）。"
  (:require [legal-entity.route :as route]
...
(defn- env->map
  ...
  （中継先 URL だけは意図的に出す。view の docstring）。"
  [env]
  (if env
...
(defn- decode-capabilities
  ...
  形を決めるのは `route/capability-labels`（純 `.cljc`、テスト対象）。ここは
  ...
  (if (string? s)
    (try (let [v (js->clj (js/JSON.parse s))]
           (if (sequential? v) (route/capability-labels v) []))
         (catch :default _ []))
    []))
  ...
  (let [e (env->map env)]
    (->response
     (view/render {:css dds-css
                   :routes route/routes
                   :capabilities (decode-capabilities (:APP_CAPABILITIES e))
